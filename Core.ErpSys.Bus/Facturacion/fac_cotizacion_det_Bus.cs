...
{
    public class fac_cotizacion_det_Bus
    {
        fac_cotizacion_det_Data oData = new fac_cotizacion_det_Data();
        ...
        public Boolean EliminarDB(int IdEmpresa, int IdSucursal, int IdPtoVenta, decimal IdCotizacion)
        {
            try
            {
                return oData.EliminarDB(IdEmpresa, IdSucursal, IdPtoVenta, IdCotizacion);
            }
            catch (Exception)
            {
        ...
        internal bool Validar_Corregir_Objeto(fac_cotizacion_Info Info, ref string MensajeError)
        {
            try
            {
                ...
                foreach (fac_cotizacion_det_Info item in Info.ListDetalle)
                {
                    if (item.IdEmpresa == 0)
                    {
                        if (Info.IdEmpresa == 0)
