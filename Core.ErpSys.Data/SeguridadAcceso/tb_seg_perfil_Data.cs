...
{
    public class tb_seg_perfil_Data
    {


        public List<tb_seg_perfil_Info> Get_List_Perfil()
        {
            List<tb_seg_perfil_Info> ListInfo = new List<tb_seg_perfil_Info>();
            ...
            try
            {
                ...
                {
                  //  ListInfo.Add(new tb_seg_perfil_Info(item.IdUsuario, item.nom_usuario, item.contrasenia, item.Estado, item.IdPerfil, item.Exigir_directivas_seguridad, item.Exigir_expiracion_contrasenia, item.usuario_debe_cambiar_contra_inicio_sesion, item.Fecha_expiracion_contrasenia));
                }

            }
            catch (Exception)
            {
                return new List<tb_seg_perfil_Info>();
            }
        ...
        public tb_seg_perfil_Info Get_Info_Perfil(string IdUsuario)
        {
            tb_seg_perfil_Info Info = new tb_seg_perfil_Info();
        ...
        public Boolean ModificarDB(tb_seg_perfil_Info Info)
        {
            try
            {
                using (Entities_Seguridad_Accesos context = new Entities_Seguridad_Accesos())
                {
                    var contact = context.seg_usuario.FirstOrDefault(cot => cot.IdPerfil == Info.IdPerfil);
